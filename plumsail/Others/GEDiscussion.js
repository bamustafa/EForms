...
var onRender = async function (moduleName, formType, relativeLayoutPath){ 
	...
	try { 
        ...
        if(moduleName == 'DBoard')
			await onDBoardRender(formType); 		
        
        const endTime = performance.now();
        const elapsedTime = endTime - startTime;
        console.log(`Execution time DBoard: ${elapsedTime} milliseconds`);
	}
	catch (e) {
		//alert(e);
		console.log(e);	
...
var onDBoardRender = async function (formType){	

    if(formType === "New")
        await DB_newForm();  
}

var DB_newForm = async function(){ 
    ...
    try 
    {
        ...
        formatingButtonsBar('Discussion Board Form');
