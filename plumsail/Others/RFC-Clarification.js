...
var onRender = async function (moduleName, formType){    
	try { 
		...
		_formType = formType;
		if(moduleName == 'RFC')
			await onRFCRender(formType);

        const endTime = performance.now();
        const elapsedTime = endTime - startTime;
        console.log(`Execution time onRFCRender: ${elapsedTime} milliseconds`);
	}
	catch (e) {
		//alert(e);
		console.log(e);	
...
var onRFCRender = async function (formType){	
	...
	if(formType === 'New'){        
		...
		await RFC_displayForm();     
    }    
}

var RFC_newForm = async function(){ 
    ...
    try {      
        ...
        ['Reference', 'Status', 'Category'].forEach(hideField); 
        ...
        const items = await pnp.sp.web.lists.getByTitle("RFCDisciplines").items.select("Category").orderBy("Category").getAll();
        const distinctValues = [...new Set(items.map((item) => item.Category))];
        fd.field('CategoryDDL').widget.setDataSource(distinctValues);
        
        fd.field('Discipline').widget.dataSource.data(['']);

        fd.field('CategoryDDL').ready().then(async function() { 

            fd.field('CategoryDDL').$on('change', async function(value) {                                         

                fd.field('Discipline').value = '';                  
                
                ['Category'].forEach(showField); 
                fd.field('Category').value = value;
                ['Category'].forEach(hideField); 

                await pnp.sp.web.lists.getByTitle("RFCDisciplines").items.select("Title").filter("Category eq '" + value + "'").orderBy("Category").get().then(async function(items) {		
                await fd.field('Discipline').widget.setDataSource({data: items.map(function(i) { return i.Title })
                });              
            });	
        });	
	});
...
var RFC_editForm = async function(){
    ...
    try {
        ...
        ['Reference', 'Status', 'TransmittalNo', 'Discipline', 'CreatedDate', 'CreatedBy', 'Title', 'ContractNo', 'Category'].forEach(disableField);        
          
        disableRichTextField('Question');
        ...
        fd.field('AnsweredBy').value = CurrentUser.LoginName;
        ['AnsweredDate', 'AnsweredBy'].forEach(disableField);    
    }    
    catch(err){
        await _generateErrorEmail(_spPageContextInfo.siteAbsoluteUrl, '', '', err.message, err.stack);        
    } 
...
var RFC_displayForm = async function(){	
...
function formatingButtonsBar(){
    ...
    $('span.o365cs-nav-brandingText').text('K23165-0102D - Request For clarification From DAR');
...
async function updateCounter() {
	...
	var camlF = "Title" + ` eq '${_ProjectNumber}-RFC'`;
	...
	await pnp.sp.web.lists.getByTitle(listname).items.select("Id, Title, Counter").filter(camlF).get().then(async function(items){
		var _cols = { };
        if(items.length == 0){
             _cols["Title"] = `${_ProjectNumber}-RFC`;
             refNo = `${_ProjectNumber}-RFC-` + String(1).padStart(3,'0'); 
            ...
            refNo = `${_ProjectNumber}-RFC-` + String(parseInt(_item.Counter)).padStart(3,'0'); ;             	
			_cols["Counter"] = value.toString();                   
