    ...
    if(missingTrades.length === 0 && PendingChampTrades.length === 0){
      ...
      if(isChamp){
        ...
        if(isSentToChampion && isChamp){
          await setButtonActions("Accept", "Validated");
          await setButtonActions("ChromeClose", "Return to GL");
        }
        else {
      ...
      if(!isChamp)
        await setButtonActions("Save", "Save");

      await setButtonActions("Accept", "Submit");
      await setButtonActions("ChromeClose", "Cancel");

      if(dimSubmit){
        $('span').filter(function () { return $(this).text() == 'Save'; }).parent().css('color', '#737373').attr("disabled", "disabled");
