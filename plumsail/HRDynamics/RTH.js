...
var onRender = async function (moduleName, formType){    
	try { 
        ...
        if(moduleName == 'RTH') // Request to Hire
			await onRTHRender(formType);
...
var onRTHRender = async function (formType){	
	...
	if(formType === 'New'){        
		...
		await RTH_displayForm();     
    }    
}

var RTH_newForm = async function(){ 
    ...
    try {      
        ...
        ['Status'].forEach(disableField); 

        setIconSource("overview-icon", "Overview.png");
        setIconSource("organization-icon", "Organization.png");
        setIconSource("dates-icon", "Dates.png");

        await loadingButtons();
        formatingButtonsBar('Human Resources: Employee Hiring Request'); 
      
        let loginName = CurrentUser.LoginName;        
        fd.field('HiringManager').value = loginName;//.UserId.NameId; //.Email;
        ['HiringManager'].forEach(disableField);         
    }    
    catch(err){
...
var RTH_displayForm = async function(){	
...
function setIconSource(elementId, iconFileName) {
    ...
    if (iconElement) {
        iconElement.src = `${_webUrl}${_layout}/Images/${iconFileName}`;
    }
}
